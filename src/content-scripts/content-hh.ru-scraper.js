(() => {
  // 🧠 Scraper script is running!
  console.log("🧠 Scraper script is running!");

  const MAX_RETRIES = 3;
  const RETRY_DELAY = 1000; // ms

  /** Utility to safely grab and trim text */
  const getText = (selector) => {
    const el = document.querySelector(selector);
    return el ? el.innerText.trim() : null;
  };

  /** Parse “от … / до …” salary snippets */
  ...
  /** Build payload object, skipping empty/null values */
  const buildPayload = () => {
    const payload = {};
    ...
    // add("experience", getText('[data-qa="vacancy-experience"]')); // ⬅️ commented out

    add("employment_type", getText('[data-qa="common-employment-text"]')); // dynamic class removed

    // const scheduleRaw = getText('[data-qa="work-schedule-by-days-text"]'); // ⬅️ commented out
    // add("schedule", scheduleRaw ? scheduleRaw.replace("График:", "").trim() : null); // ⬅️ commented out

    add("company_name", getText('[data-qa="vacancy-company-name"] span'));

    const desc = document.querySelector('[data-qa="vacancy-description"]')?.innerHTML || null; // dynamic selectors removed
    add("job_description", desc);
    ...
    // const whRaw = getText('[data-qa="working-hours-text"]'); // ⬅️ commented out
    // add("working_hours", whRaw ? whRaw.replace("Рабочие часы:", "").trim() : null); // ⬅️ commented out

    const skills = Array.from(document.querySelectorAll('[data-qa="skills-element"]')).map(
      (s) => s.innerText
    );
  ...
  /** Start once vacancy title appears */
